...
describe('McpResourcesService', () => {
  ...
  describe('handleRpcRequest', () => {
    it('should handle initialize', async () => {
      const result = await service.handleRpcRequest('user-1', { method: 'initialize' });
    ...
    it('should handle tools/list', async () => {
      ...
      const result = await service.handleRpcRequest('user-1', { method: 'tools/list' });
      expect(result).toEqual({ tools: [] });
    });

    it('should handle prompts/list', async () => {
      mockPromptRepository.find.mockResolvedValue([]);
      const result = await service.handleRpcRequest('user-1', { method: 'prompts/list' });
      expect(result).toEqual({ prompts: [] });
    });

    it('should handle resources/list', async () => {
      const result = await service.handleRpcRequest('user-1', { method: 'resources/list' });
      expect(result).toEqual({ resources: [] });
    });

    it('should return empty object for unknown method', async () => {
      const result = await service.handleRpcRequest('user-1', { method: 'unknown' });
  ...
  describe('CRUD Operations', () => {
    it('should list tools for a user', async () => {
      ...
      expect(mockToolRepository.find).toHaveBeenCalledWith({ where: { userId: 'user-1' } });
    ...
    it('should create a tool for a user', async () => {
      ...
      mockToolRepository.create.mockReturnValue({ ...toolDto, userId: 'user-1' });
      mockToolRepository.save.mockResolvedValue({ id: '1', ...toolDto, userId: 'user-1' });

      const result = await service.createTool('user-1', toolDto);

      expect(mockToolRepository.create).toHaveBeenCalledWith({ ...toolDto, userId: 'user-1' });
    ...
    it('should delete a tool for a user', async () => {
      ...
      expect(mockToolRepository.delete).toHaveBeenCalledWith({ id: 'tool-1', userId: 'user-1' });
    });

    it('should list prompts for a user', async () => {
      mockPromptRepository.find.mockResolvedValue([{ id: '1', name: 'My Prompt' }]);
      const result = await service.listPrompts('user-1');
      expect(mockPromptRepository.find).toHaveBeenCalledWith({ where: { userId: 'user-1' } });
      expect(result).toEqual([{ id: '1', name: 'My Prompt' }]);
    });

    it('should create a prompt for a user', async () => {
      const promptDto = { name: 'My Prompt', description: 'Desc', content: 'Prompt content' };
      mockPromptRepository.create.mockReturnValue({ ...promptDto, userId: 'user-1' });
      mockPromptRepository.save.mockResolvedValue({ id: '1', ...promptDto, userId: 'user-1' });

      const result = await service.createPrompt('user-1', promptDto);

      expect(mockPromptRepository.create).toHaveBeenCalledWith({ ...promptDto, userId: 'user-1' });
    ...
    it('should delete a prompt for a user', async () => {
      ...
      expect(mockPromptRepository.delete).toHaveBeenCalledWith({ id: 'prompt-1', userId: 'user-1' });
    });
  });
});
